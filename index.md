...
empirical Bayes methods for pharmacovigilance, including the
...
preprocessing, such as raw individual case safety reports (ICSRs)
...
- Nonparametric empirical Bayes methods (KM, Efron, general-gamma)
  ...
  mixture form and estimating the prior distribution in a data-driven
  way.

**Implementation highlights:**

- Provides a fully open-source KM implementation using `CVXR` (avoids
...
- Supports AIC/BIC-based hyperparameter tuning for $`\alpha`$ in the
  general-gamma model and $`(p, c_0)`$ in Efron’s approach (see Tan et
  al.  (*Stat. in Med.*, 2025) for further detail).
